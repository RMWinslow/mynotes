...
Stefan's constant is equal to $$5.670\frac{W}{m^{2}K^{4}}$$
...
$$T=\sqrt[4]{\frac{11000}{\varepsilon5.670}}K$$

So how hot does our magical griddle need to get before it emits its entire power output as radiation?

- If the object is a blackbody with $$\varepsilon = 1$$, then $$T \approx 6.7 Kelvin$$
- If the object is a graybody with emissivity [similar to stainless steel](https://www.thermoworks.com/emissivity-table) at $$\varepsilon = 0.59$$, then $$T \approx 7.6 Kelvin$$
- If the object is a graybody with emissivity [similar to polished copper](https://www.thermoworks.com/emissivity-table) at $$\varepsilon = 0.05$$, then $$T \approx 14 Kelvin$$

Wait, that's incredibly low. Is this really right? It doesn't seem right? Maybe the surface area is just so large that the temperature doesn't actually matter?
